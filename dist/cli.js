...
async function handleAccounts(args) {
    const action = args[0];
    if (!action)
        error("Missing action: list|add|remove|reauth|credentials");
    switch (action) {
        case "list": {
        ...
        case "add": {
            ...
            if (!creds)
                error("No credentials configured. Run: gmcli accounts credentials <credentials.json>");
...
async function handleSearch(account, args) {
    ...
    if (!query)
        error("Usage: <email> search <query>");
...
async function handleThread(account, args) {
    ...
    if (!threadId)
        error("Usage: <email> thread <threadId>");
...
async function handleDrafts(account, args) {
    ...
    if (!action)
        error("Usage: <email> drafts <action>");
    switch (action) {
        case "list": {
        ...
        case "get": {
            ...
            if (!draftId)
                error("Usage: <email> drafts get <draftId> [--download]");
        ...
        case "delete": {
            ...
            if (!draftId)
                error("Usage: <email> drafts delete <draftId>");
            await requireApproval({
                tool: TOOL,
                account,
        ...
        case "send": {
            const draftId = rest[0];
            if (!draftId)
                error("Usage: <email> drafts send <draftId>");
            await requireApproval({
                tool: TOOL,
                account,
                action: "drafts send",
...
async function handleSend(account, args) {
    const { values } = parseArgs({
        args,
        options: {
            to: { type: "string" },
    ...
    if (values.cc)
        details.push(`Cc: ${values.cc}`);
    if (values.bcc)
        details.push(`Bcc: ${values.bcc}`);
    details.push(`Subject: ${values.subject}`);
    if (values["reply-to"])
        details.push(`Reply to message: ${values["reply-to"]}`);
    if (values.attach?.length)
        details.push(`Attachments: ${values.attach.join(", ")}`);
...
async function describeDraft(account, draftId) {
    const draft = await service.getDraft(account, draftId);
    const headers = draft.message?.payload?.headers || [];
    const getHeader = (name) => headers.find((h) => h.name?.toLowerCase() === name.toLowerCase())?.value || "";
    const details = [`Draft: ${draftId}`, `To: ${getHeader("to")}`];
    if (getHeader("cc"))
        details.push(`Cc: ${getHeader("cc")}`);
    if (getHeader("bcc"))
        details.push(`Bcc: ${getHeader("bcc")}`);
    details.push(`Subject: ${getHeader("subject")}`);
    const attachments = getAttachments(draft.message?.payload);
    if (attachments.length > 0)
        details.push(`Attachments: ${attachments.map((a) => a.filename).join(", ")}`);
