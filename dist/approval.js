...
export async function requireApproval(req) {
    ...
    if (reason.length > MAX_REASON_LENGTH) {
        throw new Error(`Approval summary is ${reason.length} characters; the dialog can show at most ${MAX_REASON_LENGTH}. Split the action into smaller ones.`);
    }
    if (fs.existsSync(HELPER_PATH)) {
        ...
        if (result === "approved")
            return;
        if (result === "denied")
            throw new ApprovalDenied("Denied via Touch ID");
    }
        ...
        if (result === "approved")
            return;
        if (result === "denied")
            throw new ApprovalDenied("Denied via approval broker");
    }
    await askPassphrase(req.tool);
}
function runHelper(reason) {
...
async function askBroker(req, reason) {
    let response;
    try {
...
async function askPassphrase(tool) {
    const stored = readPassphraseFile();
    if (!stored) {
        throw new Error(`No approval method available: Touch ID/broker unreachable and no passphrase set. Run: ${tool} approval set-passphrase`);
    }
...
function writePassphraseFile(passphrase) {
    ...
    fs.writeFileSync(PASSPHRASE_FILE, JSON.stringify({ salt: salt.toString("hex"), hash: hashPassphrase(passphrase, salt) }, null, 2), { mode: 0o600 });
}
// --- terminal input that bypasses stdin (agents pipe stdin; humans have /dev/tty) ---
class Terminal {
    fd;
    input;
    pending = ""; // raw input received but not yet consumed by a reader
    waiter = null;
    constructor(fd) {
        this.fd = fd;
...
export async function handleApprovalCommand(tool, args) {
    switch (args[0]) {
        case "set-passphrase": {
            const term = Terminal.open();
            if (!term)
                throw new Error("A terminal is required to set the passphrase");
            try {
